...
- Preset APIs (`success`, `error`, `warning`, `info`, `loading`, `blank`, and
  `show`) no longer accept `ToastrOptions`. Their common semantic controls are
  explicit named parameters; `message` remains the only required argument.
- `ToastrOptions` is no longer exported. Use `Toastr.custom(ToastrConfig(...))`
  for colors, animations, actions, gestures, custom content, and other advanced
  customization.

### Changed

- Updated the solid semantic palette: success `#166534`, error `#B91C1C`,
  warning `#B45309`, and info `#1D4ED8`.
- Updated the example and documentation to use the new preset and custom-toast
  API boundaries.
